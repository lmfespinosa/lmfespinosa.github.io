...
LANGUAGES_ARRAY = new Array("ES","ENG");                      //all suported languages have to be written in this array. NOTE that the classnames have to be equal to these entries
FLAGS_PATH = "https://localhost/multi-language-website-JS-master/assets/img/flags/";             //the path where all the pictures of the flages are. NOTE that image name has to be (e.g. GER.jpg) like you defined it in the array "LANGUAGES_ARRAY" above
DEFAULT_LANGUAGE = "ES";                                      //set the default language
...
function showFlags(visitorsLanguage){
    ...
    for(i = 0; i<LANGUAGES_ARRAY.length; i++){ //LANGUAGES_ARRAY is the global Array where all the supportet languages are in
        ...
        if(show){
            ...
            newFlag.src = FLAGS_PATH+LANGUAGES_ARRAY[i]+".png";//link zu den Bildern
            console.log(newFlag);
